...
echo applying bpe model to input ...
subword-nmt apply-bpe -c $bpeModel  < $testFile.tok > $testFile.bpe


echo loading translation model $transModel ....
...
echo restoring segments of $testFile.trans .....
sed -r 's/(@@ )|(@@ ?$)//g' $testFile.trans > $testFile.trans.restored

echo detokensing $testFile.trans.restored .....
